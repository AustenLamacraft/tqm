...
where for brevity we write $\ket{+}_j = \ket{s^z=+1/2}_j$. It's not hard to check that this is an eigenstate with energy $E_0\equiv JN/4$. 

> $$
> \mathbf{s}_j\cdot \mathbf{s}_k = s^z_j s^z_k +\frac{1}{2}\left[s_j^+s_k^-+s_j^-s_k^+\right]
> $$
>
> where $s^\pm = s^x\pm i s^y$. 

Furthermore it's an eigenstate of $S^z=\sum_j s^z_j$ and $\mathbf{S}^2$, where $\mathbf{S}$ is the total spin
$$
\mathbf{S} = \sum_{j=1}^N \mathbf{s}_j.
$$

The eigenvalues are $S^z = N/2$ and $\mathbf{S}^2 = \frac{N}{2}\left(\frac{N}{2}+1\right)$. The rotational invariance of the Hamiltonian

> Different from the Ising model, we built the Heisenberg model out of scalar products of spin vectors.
>
> Thus an eigenstate with spin $S$ will be part of a multiplet of size $2S+1$. 

 implies that the state $\ket{\text{FM}}$ should be a member of a multiplet of $N+1$ degenerate eigenstates related by rotations. These states can be generated by acting on $\ket{\text{FM}}$ with $S^-=S^x-iS^y$. 

> $S^-=\sum_j s_j^-$ acting on the state should give us another state as $S^-$ commutes with the Hamiltonian. This is what we mean by rotational invariance (same goes for $S^+$ and $S^{z}$) 
...
<img src="IMG_0598.jpg" alt="IMG_0598" style="zoom:25%;" />

<img src="IMG_0599.jpg" style="zoom:25%;" />
...
<img src="IMG_0600.jpg" alt="IMG_0600" style="zoom:25%;" />
